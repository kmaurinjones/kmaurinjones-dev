...
>
  ...
  <p>The architecture is deliberately simple: a FastAPI backend serves a single-page application that handles all game logic in the browser. The backend's responsibilities are minimal:</p>
    ...
    <ul>
      <li><strong>Word Validation</strong>: Maintains a list of 12,966 valid five-letter words and checks if guesses are in that list</li>
      <li><strong>Solution Fetching</strong>: Retrieves the official daily solution from the NYT Wordle API for any given date</li>
      <li><strong>Guess Evaluation</strong>: Implements the authentic Wordle algorithm to determine which letters are correct (green), present but misplaced (yellow), or absent (gray)</li>
    </ul>
  ...
  <h2>Links</h2>
    ...
    <ul>
      <li><strong>Play the Game</strong>: <a href="https://wordle.kmaurinjones.dev" target="_blank" rel="noopener">wordle.kmaurinjones.dev</a></li>
